...
class ChatSessionManager:
        ...
        if storage_path.exists():
            try:
                ...
                if (
                    ...
                    or self.current_session_id not in self.sessions
                ):
                    if self.sessions:
                        # Load most recent
    ...
    def _fallback_session_name(self, first_message: str) -> str:
        """Fallback method using simple truncation of the first message"""
    ...
    def delete_session(self, session_id: str) -> bool:
        ...
        if session_id in self.sessions:
            ...
            if self.current_session_id == session_id:
                if self.sessions:
                    self.current_session_id = max(
                        self.sessions.keys(), key=lambda k: self.sessions[k].updated_at
                    )
                else:
                    self.current_session_id = self.create_new_session()
